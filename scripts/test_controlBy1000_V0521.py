...
def main():
    ...
    # 创建一个包装函数，室外温度恒定为35摄氏度
    def get_outdoor_temp(t):
        return 35.0
    
    # 为了保持代码结构，创建一个模拟的时间点和日期
    ...
    # 创建时间轴标签，修复numpy.int32转换问题，并支持多天显示
    start_time = pd.to_datetime("8/29/2007 0:00")  # 使用0:00作为起始时间
    time_labels = []
    ...
    ax1.set_title(f'{num_acs}台空调的平均室内温度和总基准功率随室外温度变化 ({days}天)')
    ax1.grid(True)
    ...
    plt.savefig(f'figures/空调温差调节_{num_acs}台_{days}天_35摄氏度_{Hm}W.png', dpi=300)
    plt.show()
    
    print(f"模拟完成，图表已保存为 'figures/空调温差调节_{num_acs}台_{days}天_35摄氏度_{Hm}W.png'")
