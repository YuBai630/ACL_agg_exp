...
def adjust_migration_probabilities_with_target(etp_model, t_indoor, t_indoor_mass, t_outdoor, 
                                            ...
                                            cop, tonlock_sec, tofflock_sec, sim_dt, current_sim_time=0):
    """
    ...
    tofflock_sec: 关锁时间 [秒]
    sim_dt: 仿真步长 [秒]
    current_sim_time: 当前模拟时间 [秒]，用于判断是否处于需求响应时段
    
    返回:
    adjusted_u0: 调整后的u0值
    adjusted_u1: 调整后的u1值
    fsm_driving_power: 实际用于计算u0,u1的功率 (可能已打折) [W]
    original_required_power: 未经需求响应打折的原始所需功率 [W]
    """
    # 计算目标温度区间
    ...
    if t_indoor < t_min:  # 室内温度低于下限，需要减少制冷
        ...
        original_required_power = 0  # 不需要制冷
        fsm_driving_power = original_required_power  # 确保fsm_driving_power被定义
        # 关闭概率=1，开启概率=0
        u0, u1 = 1.0, 0.0  
        
    elif t_indoor > t_max:  # 室内温度高于上限，需要增加制冷
        # 计算达到目标温度下限所需的功率
        original_required_power, _, _ = calculate_power_for_target_temp(
            etp_model, t_indoor, t_indoor_mass, t_min, 
            t_outdoor, control_cycle_sec, cop
        )
        # 确保功率不超过额定功率
        original_required_power = min(original_required_power, rated_power)
        
        # 应用需求响应折扣 (先复制原始功率)
        fsm_driving_power = original_required_power
        
        # 检查是否在需求响应时段 (11:00 - 14:00)
        current_hour = (current_sim_time % (24 * 3600)) // 3600
        if 11 <= current_hour < 14:
            fsm_driving_power *= 0.3  # 需求响应，功率降低至30%
        
        # 使用FSM计算迁移概率 (使用可能已打折的功率)
        u0, u1 = temp_fsm.calculate_migration_probabilities(fsm_driving_power, rated_power)
    ...
    return adjusted_u0, adjusted_u1, fsm_driving_power, original_required_power
...
class AggregationController:
    ...
    def update_control_cycle(self, t_outdoor, control_cycle_sec, current_sim_time):
        """在控制周期开始时更新所有空调的控制参数
        
        参数:
        t_outdoor: 室外温度 [°C]
        control_cycle_sec: 控制周期时间 [秒]
        current_sim_time: 当前模拟时间 [秒]
        ...
        total_required_power_before_dr: 未打折扣的总所需功率 [W]
        """
        # 计算每台空调的基准功率
        total_base_power = 0
        ...
        # 新增：未打折扣的总所需功率
        total_required_power_before_dr = 0
        
        # 更新所有空调的迁移概率
        for ac_idx in range(self.num_acs):
            # 使用基于目标温度的方法计算迁移概率
            adjusted_u0, adjusted_u1, fsm_driving_power, original_required_power = adjust_migration_probabilities_with_target(
            ...
            # 累加未打折扣的所需功率
            total_required_power_before_dr += original_required_power

            # 下面的需求响应折扣代码块已经被移到 adjust_migration_probabilities_with_target 函数中
            # 计算当前时间是否在需求响应时段 (11:00 - 14:00)
            # 需要考虑多天的情况
            current_hour = (current_sim_time % (24 * 3600)) // 3600
            if 11 <= current_hour < 14 and ac_idx == 0:
                print(f"时间 {current_sim_time/3600:.2f}h: 处于需求响应时段，空调1的所需功率降低至30%")
            ...
            if ac_idx == 0:
                ...
                print(f"空调1的迁移概率: u0={original_u0:.4f}(关闭概率), u1={original_u1:.4f}(开启概率), 需求功率比例：{fsm_driving_power/rated_power:.4f}")
            
            # 累加响应功率和平均迁移概率
            total_response_power += fsm_driving_power
            avg_u0 += adjusted_u0
            avg_u1 += adjusted_u1
        ...
        return total_base_power, avg_u0, avg_u1, total_response_power, total_required_power_before_dr
...
def main():
    ...
    # 新增：存储总瞬时功率和需求响应功率历史
    total_instantaneous_power_history = np.zeros(len(time_points))
    demanded_power_history = np.zeros(len(time_points))
    # 新增：存储未打折扣的总所需功率历史
    required_power_before_dr_history = np.zeros(len(time_points))
    ...
    for t_idx, t in enumerate(time_points):
        ...
        if t - last_control_update_time >= control_cycle_sec or t_idx == 0:
            ...
            # 更新聚合商控制参数
            total_base_power, avg_u0, avg_u1, total_response_power, total_required_power_before_dr = aggregator.update_control_cycle(
                Tout, control_cycle_sec, t
            )
            ...
            # 更新当前需求响应功率（单位：W）
            current_demanded_power = total_response_power
            
            # 存储未打折扣的总所需功率（转换为kW）
            current_required_power_before_dr = total_required_power_before_dr / 1000
        ...
        # 存储当前需求响应功率（转换为kW）
        demanded_power_history[t_idx] = current_demanded_power / 1000
        
        # 存储未打折扣的总所需功率（转换为kW）
        required_power_before_dr_history[t_idx] = current_required_power_before_dr
        ...
        # 计算当前总瞬时功率（单位：W）
        current_instantaneous_power = 0
        physical_states = aggregator.get_physical_states()
        for ac_idx in range(aggregator.num_acs):
            if physical_states[ac_idx] == 1: # 1 表示空调开启
                # 获取该空调的制冷功率 (Hm 的绝对值) 并除以 COP
                cooling_capacity = abs(aggregator.etp_models[ac_idx].Hm)
                cop = aggregator.cop_values[ac_idx]
                current_instantaneous_power += cooling_capacity / cop
                # current_instantaneous_power += cooling_capacity
                
        # 存储总瞬时功率（转换为kW）
        total_instantaneous_power_history[t_idx] = current_instantaneous_power / 1000
    ...
    # 控制台输出demanded_power_history和required_power_before_dr_history的差值
    # diff_power_history = demanded_power_history - required_power_before_dr_history
    # print("demanded_power_history 与 required_power_before_dr_history 的差值（单位：kW）：")
    # 保存总瞬时功率历史到figures目录下的CSV文件
    
    figures_dir = "figures"
    os.makedirs(figures_dir, exist_ok=True)
    # 保存总功率历史
    csv_path = os.path.join(figures_dir, "total_instantaneous_power_history_dr.csv")
    pd.DataFrame({"total_instantaneous_power_kW": total_instantaneous_power_history}).to_csv(csv_path, index=False, encoding="utf-8-sig")
    print(f"总瞬时功率历史已保存到: {csv_path}")
    # 补充：保存demanded_power_history
    demanded_power_csv_path = os.path.join(figures_dir, "demanded_power_history_dr.csv")
    pd.DataFrame({"demanded_power_kW": demanded_power_history}).to_csv(demanded_power_csv_path, index=False, encoding="utf-8-sig")
    print(f"需求响应目标功率历史已保存到: {demanded_power_csv_path}")
    ...
    # 标记需求响应时段 (11:00 - 14:00)
    for day in range(int(days)):
        dr_start_time_sec = day * day_time + 11 * 3600  # 每天的11:00
        dr_end_time_sec = day * day_time + 14 * 3600    # 每天的14:00
        
        dr_start_datetime = start_time + timedelta(seconds=int(dr_start_time_sec))
        dr_end_datetime = start_time + timedelta(seconds=int(dr_end_time_sec))
        
        for ax in [ax1, ax2]:
            ax.axvspan(dr_start_datetime, dr_end_datetime, color='orange', alpha=0.3, label='需求响应时段' if day == 0 else "")
    ...
    # 新增：绘制总瞬时功率和需求响应功率
    ax1b.plot(time_labels, total_instantaneous_power_history, 'm-', label='实际总功率')
    ax1b.plot(time_labels, demanded_power_history, 'c--', label='需求响应目标功率')
    # 新增：绘制未打折扣的总所需功率
    ax1b.plot(time_labels, required_power_before_dr_history, 'y--', label='原始所需功率')
    ax1b.set_ylabel('功率 (kW)', color='g') # 修改Y轴标签
    ...
    fig_path= f"figures/聚合商控制_{num_acs}台空调_{days}天_agg_wdr.png'"
    plt.savefig(f'figures/聚合商控制_{num_acs}台空调_{days}天_agg_wdr.png', dpi=300)
    plt.show()
    
    print(f"模拟完成，图表已保存为 '{fig_path}'")
